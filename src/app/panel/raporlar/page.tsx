...
export default async function RaporlarPage({
  ...
  searchParams: Promise<{ sekme?: string; aralik?: string; grup?: string }>;
}) {
  const { sekme: sekmeParam, aralik: aralikParam, grup: grupParam } = await searchParams;
  const sekme = sekmeParam === "servis" ? "servis" : "satis";
    ...
    <div className="mx-auto max-w-5xl">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-400">Satış ve servis performansı</p>
        </div>
      </div>

      {/* Aralık seçici — link seti (JS gerektirmez) */}
      ...
      <div className="mt-5">
        {sekme === "satis" ? satisIcerik : servisIcerik}
      </div>
    </div>
  );
}
