...
const s = StyleSheet.create({
  ...
  colBakiye: { width: 70, textAlign: "right", fontWeight: 700 },
  altBilgi: { position: "absolute", bottom: 30, left: 36, right: 36 },
});

type Musteri = { name: string; phone: string | null; balance: number };
...
const ENTRY_ETIKET: Record<string, string> = {
  acik_hesap_satis: "Açık hesap satış",
  tahsilat: "Tahsilat",
  duzeltme: "Düzeltme",
...
export function MusteriEkstresi({
  musteri,
  isletme,
  hareketler,
}: {
  ...
  return (
    <Document title={`${musteri.name} — Cari Ekstre`}>
      <Page size="A4" style={s.page}>
        <View style={s.ustBaslik}>
          ...
          <View>
            <Text style={s.belgeBaslik}>CARİ EKSTRE</Text>
        ...
        <View style={{ flexDirection: "row", marginBottom: 14, gap: 10 }}>
          ...
          <View style={s.ozetKutu}>
            ...
            <Text style={s.ozetDeger}>{tl(musteri.balance)}</Text>
          </View>
        </View>
          ...
          {hareketler.length === 0 ? (
            ...
            hareketler.map((h, i) => (
              <View key={i} style={s.tabloSatir}>
                ...
                <Text style={[s.tabloHucre, s.colTutar]}>{h.amount > 0 ? h.amount.toLocaleString("tr-TR") : ""}</Text>
                <Text style={[s.tabloHucre, s.colTutar]}>{h.amount < 0 ? Math.abs(h.amount).toLocaleString("tr-TR") : ""}</Text>
                <Text style={[s.tabloHucre, s.colBakiye]}>{tl(h.balance_after)}</Text>
              </View>
            ))
          )}
        </View>
        ...
        <View style={s.altBilgi} fixed>
          <Text style={{ fontSize: 7, color: "#999", textAlign: "center" }}>
            {musteri.name} · Düzenlenme: {tarih(new Date().toISOString())} · ByteNova ile oluşturulmuştur
          </Text>
        </View>
      </Page>
    </Document>
